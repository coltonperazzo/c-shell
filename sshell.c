        ...
        if (strlen(prog) == strlen(cmd)) { new_cmd.args[1] = NULL; } // If command has no args, set second arg to NULL (first arg is program name).
                ...
                while (cmd_arg != NULL) {
                        ...
                        if (can_add_arg) { 
                                ...
                                // Set into array and increase amount of args.
                                new_cmd.args[new_cmd.number_of_args] = cmd_arg;
                                new_cmd.number_of_args = new_cmd.number_of_args + 1;
                        }
                        cmd_arg = strtok(NULL, " "); // Split into next part of character.
                }
                new_cmd.args[new_cmd.number_of_args] = NULL; // Always set last arg in array to NULL (required for exec).
        }
        return new_cmd;
}
        ...
        while (1) {
                ...
                /*else if (!strcmp(cmd, "cd")) { 
                        ...
                        if (has_multiple_commands) { // We must have multiple commands.
                                ...
                                if (can_run) { // Can this command run (check parsing errors/etc).
